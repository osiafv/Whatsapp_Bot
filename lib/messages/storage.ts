...
export class Storager extends UserHandler {
    ...
    private async SaveMedia() {
        globalThis.CMD.on('storage|save <id, media>',   { event: ["save <id, media>"], tag: "storage"},'save', async (res: WAConnection, data: Commands) => {
    ...
    private async checkMedia() {
        globalThis.CMD.on('storage|check <id>',   { event: ["check <id>"], tag: "storage"} ,'check', (res: WAConnection, data: Commands) => {
    ...
    private async getMedia() {
        globalThis.CMD.on('storage|get <id>',  { event: ["get <id>"], tag: "storage"}, 'get', async (res: WAConnection, data: Commands) => {
